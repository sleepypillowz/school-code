...
int main(void)
{
  ...
  for (int i = 0; i < height; i++) // loops until i is equal to height; this tells the height of the object
  {
    ...
    // spacing to make it it right aligned
    for (int j = (height - 1); j > i; j--) // used height so spacing adjusts and -1 to remove space at the bottom
    {
      printf(" ");
    }
    for (int k = 0; k <= i; k++) // loops until k is less than or equal to i; this tells the length of the object
    {
      printf("#");
    }

    printf("  "); // add two spaces between the two halves of the pyramid
